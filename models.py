...
from pann.models import Cnn14_DecisionLevelMaxMels, Cnn14_DecisionLevelMax
...
class FC(nn.Module):
    def __init__(self, scores_len, dtype=torch.FloatTensor):
        super().__init__()
    ...
    def forward(self, x):
        ...
        #MLP version
        x_interm = F.relu(self.input_fc(x))
        y_pred = self.m(self.output_fc(x_interm))
...
class PANNPINV(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(f'duration pinv: {duration}')
        # start_time = time.time()
        y_pred = self.model(y_fc)['framewise_output'].mean(dim=1)
        # duration = time.time() - start_time
        # print(f'duration model: {duration}')
        return y_pred
