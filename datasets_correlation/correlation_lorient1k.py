...
def weighted_top_10(arr, idx_t=300, idx_v=0, idx_b=111):
    ...
    top = 10
    t = (1 - sorted_t/top) if sorted_t < top else 0
    v = (1 - sorted_v/top) if sorted_v < top else 0
    b = (1 - sorted_b/top) if sorted_b < top else 0
...
def compute_groundtruth():
    ...
    data = np.load("./LORIENT1K_DATASET/Lorient-1k_time_of_presence.npy")
    names = np.array(['00'+str(k) if len(str(k))==1 else '0'+str(k) for k in range(1, len(data)+1)])
                ...
                if ((classifier in ["PANN", "YamNet",  "CNN-PINV-PANN",  "CNN-PINV-YamNet"]) & (deep=="False")):
                    if (classifier in ["PANN", "CNN-PINV-PANN"]):
                        if to_tvb:
                            ...
                            #t: 513 --> noise , 351 --> engine starting
                            #v: 0 --> speech
                            #b: 114 --> squawk
                            t = temp[327]
                            v = temp[0]
